...
import exceptions.{NotSquareMatrixException, SizeException}
...
object Matrix {
  ...
  def identity(size: Int): Matrix = new Matrix ((for (i <- 0 to size) yield {(for (j <- 0 to size) yield {if i == j then 1 else 0}).toList}).toList)
